...
describe('Unit Test: Update Last Annotation Content', () => {
	test("add anchors and update content", async done => {
		...
		const getResponse = await request(app).put(`${service}/${testAnchor.anchorId}/update`).send({annotation: "Hello there", author: "Jinoo Hong"}).expect(200).expect('Content-Type', /json/)
		...
		expect(sr.payload.contentList).toEqual(["Hello there"])
		expect(sr.payload.authorList).toEqual(["Jinoo Hong"])
