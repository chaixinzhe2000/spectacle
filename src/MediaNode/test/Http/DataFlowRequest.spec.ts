...
describe('Unit Test: Get Node', () => {  
  ...
  test("deletes document", async done => {
    try {
      const deleteResponse = await request(app).delete('/immutable-text/testid').expect(200).expect('Content-Type', /json/)
  ...
  test("deletes document", async done => {
    ...
    const createResponse = await request(app).post('/immutable-text').send({data: {
          nodeId: 'testid',
          text: 'test string'
  ...
  test("deletes document", async done => {
    ...
    const getResponse = await request(app).get('/immutable-text/testid').expect(200).expect('Content-Type', /json/)
  ...
  test("deletes document", async done => {
    const deleteResponse2 = await request(app).delete('/immutable-text/testid').expect(200).expect('Content-Type', /json/)
    expect(isServiceResponse(deleteResponse2.body)).toBeTruthy()
    done()
  })

  test("deletes document", async done => {
    const getResponse = await request(app).get('/immutable-text/testid').expect('Content-Type', /json/).expect(200)
    expect(isServiceResponse(getResponse.body)).toBeTruthy()
    done()
  })
})
